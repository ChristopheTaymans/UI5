		...
		return BaseController.extend("SAPPOApproval.Application.controllers.Master", {
			...
			onSave: function () {
				...
				this.getModel("main").submitChanges({
					success: function (oData) {						
						this.setBusy(false);
						if (!this.getModel("message").getData()) {
							MessageToast.show("Saved...");
							this.setInfo("toSave", false);
							this.setInfo("createMode", false);
			...
			_getMessagePopover: function () {
				...
				if (!this._oMessagePopover) {
					this._oMessagePopover = sap.ui.xmlfragment(this.getView().getId(), "SAPPOApproval.Application.fragment.MessagePopover", this);
			...
			onLifnrSearch: function (oEvt) {
				// add filter for search
				var aFilters = [];
				var sQuery = oEvt.getSource().getValue();
				if (sQuery && sQuery.length > 0) {
					var filter = new Filter("Lifnr", sap.ui.model.FilterOperator.Contains, sQuery);
					aFilters.push(filter);
				};
				// update list binding
				var list = this.getView().byId("lstPO");
